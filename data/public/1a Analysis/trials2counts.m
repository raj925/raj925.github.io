...
%     end
    ...
    for r = nRatings : -10 : 0
        nR_S1(end+1) = sum(stimID==0 & response==0 & rating<=r & rating > r-10);
        nR_S2(end+1) = sum(stimID==1 & response==0 & rating<=r & rating > r-10);
    end
    
    nR_S1_size = size(nR_S1);
    nR_S2_size = size(nR_S2);
    nR_S1_size = nR_S1_size(2);
    nR_S2_size = nR_S2_size(2);
    
    nR_S1(nR_S1_size-1) = nR_S1(nR_S1_size) + nR_S1(nR_S1_size-1);
    nR_S2(nR_S2_size-1) = nR_S2(nR_S2_size) + nR_S2(nR_S2_size-1);
    nR_S1(nR_S1_size) = [];
    nR_S2(nR_S2_size) = [];

    for r = 1 : 10: nRatings
        nR_S1(end+1) = sum(stimID==0 & response==1 & rating>=r & rating < r+10);
        nR_S2(end+1) = sum(stimID==1 & response==1 & rating>=r & rating < r+10);
    end
